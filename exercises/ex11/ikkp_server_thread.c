...
pthread_t make_thread(void *(*connection_handler)(void*), void* client_sock)
{
    int ret;
    pthread_t thread;

    ret = pthread_create(&thread, NULL, connection_handler, (void *) client_sock);
    if (ret != 0) {
...
void *connection_handler(void *socket_desc)
{
    ...
    int connect_d = *(int*)socket_desc;   //taking out the first * takes down the server
    printf("%i", connect_d);
    ...
    while (strncasecmp("Surrealist giraffe.", buf, 12)) {
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        while(connect_d){
            ...
            make_thread(connection_handler, &connect_d);
